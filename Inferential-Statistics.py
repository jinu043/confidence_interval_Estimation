...
st.markdown("<h5 style='text-align: justify'>Confidence interval estimation is a technique from inferential statistics method for calculating the \
 population parameters range such as mean, variance, proportion etc. My intention is to calculate the average price and average mileage \
 of used cars located in united arab emirates. I have collected data from various web sites of established dealers in UAE market \
 by using python web scrapping library (Selenium).</h5>", unsafe_allow_html=True)

st.markdown("<h5 style='text-align: justify'>Since I collected the samples from various sources, we could calculate only \
sample statistics such as sample standard deviation, sample mean, etc and therefore we have to use t-distribution to calculate the \
